    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-100 dark:border-gray-700 p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
      ...
      <div className="flex-grow grid grid-cols-1 sm:grid-cols-12 gap-4 w-full items-center">
        <div className="sm:col-span-3">
          <h3 className="font-bold text-gray-800 dark:text-gray-100">{addon.name}</h3>
          <div className="flex flex-wrap gap-1 mt-1 sm:hidden">
             {addon.tags?.map(tag => (
               <span key={tag} className="text-[10px] bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-1.5 rounded-sm">
                 {tag}
               </span>
             ))}
          </div>
        ...
        <div className="sm:col-span-2 text-sm text-gray-500 dark:text-gray-400 flex items-center">
        ...
        <div className="sm:col-span-4 flex flex-col justify-center">
          <span className="text-sm text-gray-600 dark:text-gray-300 line-clamp-2 sm:line-clamp-1 mb-1">
            {addon.description}
          </span>
          <div className="hidden sm:flex flex-wrap gap-1">
             {addon.tags?.slice(0, 4).map(tag => (
               <span key={tag} className="text-[10px] bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border border-gray-100 dark:border-gray-600 px-1.5 rounded-full">
                 {tag}
               </span>
             ))}
          </div>
